...
//#addin nuget:?package=NuGet.CommandLine&version=5.8.1
	...
	.Does(() =>
{
	...
	var javaExe = MakeAbsolute(new DirectoryPath(JAVA_HOME).Combine("bin/java")).FullPath;
	...
	() =>
	{
		...
		foreach(string config in Configs)
		{
			foreach (var sampleSln in sampleSlns)
			{
				...
				// or be installed as Cake addin using NuGet.CommandLine package.
